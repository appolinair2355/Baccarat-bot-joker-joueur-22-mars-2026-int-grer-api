...
# Compteur2 - comptage des apparitions par costume chez le joueur
...
async def update_prediction_message(game_number: int, status: str, trouve: bool, rattrapage: int = 0):
    ...
    _digit_emoji = {0: "0️⃣", 1: "1️⃣", 2: "2️⃣", 3: "3️⃣"}
    if trouve:
        digit = _digit_emoji.get(rattrapage, str(rattrapage))
        result_line = f"✅{digit}GAGNÉ"
    else:
        result_line = "❌PERDU"
...
# ============================================================================
# COMPTEUR2 - Logique principale (comptage costumes du joueur)
# ============================================================================
...
def get_compteur2_status_text() -> str:
    ...
    lines = [
        ...
        "",
        "Comptage costumes joueur (depuis dernier reset horaire):",
    ]
...
async def process_compteur2(game_number: int, player_suits: List[str], player_cards: list):
    ...
    Nouvelle logique :
    - Compte toutes les apparitions de chaque costume dans les cartes du joueur
      (ex: ♠️♠️ = +2 pour ♠)
    - Pour chaque paire inverse (♠/♦ et ♥/♣), si la différence de comptage >= BP :
      → Prédire le costume le moins vu pour le prochain jeu
...
async def api_polling_loop():
    ...
    - Compteur2 + Prédiction : déclenché dès que le joueur a FINI de tirer ses cartes
      (pas besoin d'attendre la fin complète de la partie côté banquier).
        ...
        try:
            ...
            if results:
                for result in results:
                    ...
                    # Extraire costumes joueur (uniques, pour la vérification)
                    ...
                    # Déclenché dès que le joueur a FINI de tirer ses cartes
                    # (n'attend PAS la fin complète de la partie côté banquier)
                    drawing_done = _player_drawing_done(player_cards)
                    if game_number not in player_prediction_triggered and drawing_done:
                        ...
                        logger.info(
                            f"🃏 Joueur #{game_number} FINI de tirer | Main: {p_display} "
                            f"| is_finished={is_finished}"
                        )
                        # Compter les costumes joueur et déclencher la prédiction du JEU SUIVANT
                        await process_compteur2(game_number, player_suits, player_cards)
